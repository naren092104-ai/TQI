...
export interface Student { id: ID; name: string; rollNo: string; schoolId: ID; grade: string; gender: "M" | "F"; guardian: string; phone: string; }
...
export interface Bill { id: ID; name: string; url: string; type: "uploaded" | "scanned"; }
export interface Expense { id: ID; date: string; category: "Travel" | "Food" | "Stationery" | "Other"; amount: number; description: string; submittedBy: string; status: "Pending" | "Approved" | "Rejected"; bills: Bill[]; advanceId?: ID; remarks?: string; }
  ...
  init: async () => {
    ...
    try {
      const entries = await Promise.all(
        (Object.keys(seed()) as Array<keyof DB>).map(async (resource) => {
          const items = await fetchResource(resource as string);
          return [resource, items] as const;
        }),
      );
      const loaded = entries.reduce((acc, [key, value]) => ({ ...acc, [key]: value }), {} as DB);
      set({ ...seed(), ...loaded, initialized: true });
    } catch (error) {
