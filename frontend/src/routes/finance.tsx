...
function Page() {
  ...
  const [form, setForm] = useState<{ date: string; category: Expense["category"]; amount: number; description?: string; submittedBy: string; spocName?: string; financerName?: string; volunteerCount?: number; travelFrom?: string; travelTo?: string; breakfast?: number; lunch?: number; dinner?: number; refreshment?: number; bills: Bill[]; clusterId?: string; villageId?: string; schoolId?: string }>({
    date: "", category: "Travel", amount: 0, submittedBy: "", spocName: "", financerName: "", volunteerCount: 0, travelFrom: "", travelTo: "", breakfast: 0, lunch: 0, dinner: 0, refreshment: 0, bills: [], clusterId: "", villageId: "", schoolId: "",
  });
  ...
  const openCreate = () => { setEdit(null); setForm({ date: new Date().toISOString().slice(0,10), category: "Travel", amount: 0, submittedBy: currentUser, spocName: "", financerName: "", volunteerCount: 0, travelFrom: "", travelTo: "", breakfast: 0, lunch: 0, dinner: 0, refreshment: 0, bills: [], clusterId: "", villageId: "", schoolId: "" }); setOpen(true); };
  const openEdit = (e: Expense) => { setEdit(e); setForm({ date: e.date, category: e.category, amount: e.amount, description: e.description ?? "", submittedBy: e.submittedBy, spocName: (e as any).spocName ?? "", financerName: (e as any).financerName ?? "", volunteerCount: (e as any).volunteerCount ?? 0, travelFrom: e.travelFrom ?? "", travelTo: e.travelTo ?? "", breakfast: e.breakfast ?? 0, lunch: e.lunch ?? 0, dinner: e.dinner ?? 0, refreshment: e.refreshment ?? 0, bills: e.bills, clusterId: "", villageId: "", schoolId: "" }); setOpen(true); };
  const save = () => {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div><Label>SPOC Name</Label><Input value={form.spocName || ""} onChange={(e) => setForm({ ...form, spocName: e.target.value })} placeholder="SPOC name" /></div>
            <div><Label>Financer Name</Label><Input value={form.financerName || ""} onChange={(e) => setForm({ ...form, financerName: e.target.value })} placeholder="Financer name" /></div>
            ...
            {form.category === "Travel" ? (
              <>
                ...
                <div><Label>Number of Volunteers</Label><Input type="number" min={0} value={form.volunteerCount || 0} onChange={(e) => setForm({ ...form, volunteerCount: Number(e.target.value) })} /></div>
